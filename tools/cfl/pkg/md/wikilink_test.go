...
func TestConvertACLinksToWikiLinks(t *testing.T) {
	tests := []struct {
		name     string
		html     string
		expected string
	}{
		{
			...
			expected: `<p>See [[My Page]] for details.</p>`,
		},
		{
			name: "cross-space link",
			html: `<p>Check <ac:link><ri:page ri:content-title="Architecture" ri:space-key="DEV" />` +
				`<ac:plain-text-link-body><![CDATA[Architecture]]></ac:plain-text-link-body></ac:link> page.</p>`,
			expected: `<p>Check [[DEV:Architecture]] page.</p>`,
		},
		{
			name:     "link without body element",
			html:     `<ac:link><ri:page ri:content-title="No Body" /></ac:link>`,
			expected: `[[No Body]]`,
		},
		{
			name:     "no ac:link elements",
			html:     `<p>Just plain HTML</p>`,
			expected: `<p>Just plain HTML</p>`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := convertACLinksToWikiLinks(tt.html)
			assert.Equal(t, tt.expected, result)
		})
	}
}
...
func TestRoundtrip_WikiLinks_WithMacros(t *testing.T) {
	// Wiki links + macros should both survive roundtrip
	input := "[TOC]\n\nSee [[My Page]] for details.\n\n[INFO]\nImportant info about [[DEV:Architecture]]\n[/INFO]"

	storage, err := ToConfluenceStorage([]byte(input))
	require.NoError(t, err)

	// Verify both macros and wiki links are present
